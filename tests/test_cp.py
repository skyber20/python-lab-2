...
def test_cmd_cp(fs: FakeFilesystem, mock_logger_info: Mock):
    ...
    assert 'cp: OK' in mock_logger_info.call_args_list[-1].args[0]
...
def test_cmd_cp_errors(fs: FakeFilesystem, mock_logger_error: Mock, mock_print: Mock):
    ...
    assert 'cp: Некорректное количество путей' in str(mock_print.call_args_list[-1].args[0])
    assert 'cp: Некорректное количество путей' in str(mock_logger_error.call_args_list[-1].args[0])

    error = run_command('cp', ['test/file_in.txt', 'test2', '-f'])[1]
    assert isinstance(error, InvalidOption)
    assert 'cp: Опция -f не поддерживается' in str(mock_print.call_args_list[-1].args[0])
    assert 'cp: Опция -f не поддерживается' in str(mock_logger_error.call_args_list[-1].args[0])
    ...
    assert 'Для копирования папки нужна опция -r' in str(mock_print.call_args_list[-1].args[0])
